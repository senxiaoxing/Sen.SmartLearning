 ...
 * | 场景 | 行为 |
 * |---|---|
 * | 缓存齐全（绝大多数启动） | 审计约几百毫秒，全程不渲染任何东西 |
 * | 有 SW 接管、缺了一部分 | 弹层 + 主动逐条拉取（fetch 即缓存），补齐自动消失 |
...
export function VoiceCacheGate() {
  ...
  useEffect(() => {
    // 开发模式没有 SW，审计永远 0，不能拦
    if (import.meta.env.DEV) return

    let cancelled = false

    const run = async () => {
      const keys = Object.keys(VOICE_MANIFEST)
      let report = await auditVoiceCache(keys)

      while (!cancelled && report.cached < report.total) {
        const before = report.cached
        setFilling({ cached: report.cached, total: report.total, stalled: false })

        if (navigator.serviceWorker?.controller != null) {
          // SW 已接管：主动补录。fetch 的进度实时推给进度条，
          // 但以下一轮 audit 的结果为准——fetch 成功不等于真的进了缓存
          await repairVoiceCache(report.missing, (done) => {
            if (!cancelled) {
              setFilling({
                cached: Math.min(report.total, before + done),
                total: report.total,
                stalled: false,
              })
            }
        ...
        report = await auditVoiceCache(keys)
        if (cancelled) return

        if (report.cached <= before && report.cached < report.total) {
          // 一整轮毫无进展：断网，或托管端出了问题。亮出提示后歇一会再试
          setFilling({ cached: report.cached, total: report.total, stalled: true })
          await sleep(RECHECK_DELAY_MS)
          report = await auditVoiceCache(keys)
        }
      }

      if (!cancelled) setFilling(null)
    }
  ...
  const ratio = filling.total === 0 ? 0 : filling.cached / filling.total
