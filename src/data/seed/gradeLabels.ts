 ...
 *
 * 数字她在识字第一辑第二组就学过，数学题和首页也天天见；
 * 而识字墙、诗单的分辑切换用的正是同一套徽记 + 同一个 `VolumePicker`。
 * 她在那两处学会的「按数字换一批」这个动作，到这里要能原样再用一次。
 *
 * ⚠️ 这里是**朴素数字字符**，不是 emoji 数字（`1️⃣`）。徽章的样子由
 * `VolumePicker` 的 `Badge` 画。emoji 在 iPad 和 Windows 上是两套完全不同的画，
 * 开发机看到的和孩子看到的对不上，那个蓝底白字方块也跟三套皮肤都不搭。
 */
