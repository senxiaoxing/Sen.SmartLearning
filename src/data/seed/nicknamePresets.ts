 ...
 * ## 这份清单就是昵称的全部选项
 *
 * 家长区**只能从本清单里挑**（NicknameSetting），不再自由输入——
 * 预生成音频是构建期的事，运行时凭空造不出「小恩宝」三个字的少女声片段，
 * 而清单外的昵称会让每句问候整句降级成机器音。
 *
 * 降级路径仍然保留（`platform/speech.ts` 的 `say()` 天然支持），
 * 但只服务一种情况：**改版前用键盘存下的旧昵称**。
 *
 * ```
 * 昵称在本清单里（正常路径）   → 整句都是预生成片段    「小恩宝，太棒了」全程少女声 ✅
 * 旧档案遗留的自由昵称         → 整句降级为实时 TTS    音色不如片段，但名字照样叫得出来
 * ```
 *
