/**
 * @file 拼音音节表 —— 每个音节念什么、它的汉字示例是什么
 * @layer data  静态内容，随 App 版本内置
 * @see design/11-拼音真人录音方案.md  ⭐ 音频来源（2026-09 起改为真人录音）
 * @see design/07-音频方案.md §3.3 拼音音频（TTS 时代的完整记录，含改版经过）
 ...
 *
 * ## ⚠️ `char` 不再决定音频了（2026-09）
 *
 * 从前音频由 Edge TTS 在构建期生成，而 TTS 是**文本**转语音：
 * 喂 `bā` 它只能猜，喂汉字「八」才必然读对。所以每个音节都得挂一个汉字载体，
 * 上面那三条选字规矩就是为它定的。
 *
 * 现在**音频全部来自外部真人录音**（`npm run pinyin:voice`，见 design/11），
 * 载体字不再参与发声。但 `char` 仍有三处用途，**不能删**：
 *
 * ```
 * pinyinChart.ts 的 spoken     片段缺失时走 TTS 的兜底文本 —— 「玻」必然读对，
 *                              而拿 'b' 去念会得到英文字母 bee
 * pinyinChart.ts 的 carrier    整体认读卡要把它显示出来（卡面写 zhi、念的是「知」）
 * isCharUsable()               P8.3「听音选字」的选项池
 * ```
 *
 * ## 选字的三条规矩仍然有效
 *
 * 只是约束的对象从「音频」变成了「兜底文本与选题」：
 *
 * 1. **绝不用多音字** —— 「和」有 hé/huó/hè/huo 四读，兜底 TTS 挑哪个全凭运气。
 *    这是本表最大的风险来源。
 * 2. **优先常用字** —— 生僻字 TTS 也可能读错，且无法靠常识复核。
 * 3. **`char` 与 `pinyin` 必须真的对应** —— 这一列仍是给人复核用的：
 *    懂拼音的人扫一眼「bā → 八」就知道对不对，不必听音频。
 *
 * 找不到合适汉字的音节 `char` 留空（现在只剩 `ong` 一条）：
 * `isUsable()` 为 false，一律不进题库。
 */
 ...
 * 声母表（23 条）。
 *
 * ⚠️ **题目念的是呼读音**（b 念「玻」bo，选项显示 `bō`）——
 * 而**拼音墙念的是本音**（卡面写 `b`，播 `pinyinbare.b`）。
 * 两条轨道是刻意的：`pinyin.fo2` 同时被题目当作「fo 这个音节」使用，
 * 就地换成纯辅音会让那些题没有正确答案。见 design/11-拼音真人录音方案.md §3.1。
 *
 * ⭐ **d n l c 只能用教学惯用字**（的 讷 乐 呲）——它们是多音或生僻，
 * 但读音必须和老师教的一模一样，这一点压倒「用常用字」的偏好。
 * 全部标 `soundOnly`：不会出现在「听音选字」的选项里（见 domain/pinyin.ts）。
 */
 ...
 *
 * ⚠️ 换真人录音之后（2026-09）这条设计**依然成立**：它管的是
 * 「屏幕与语音声明什么」，与音频从哪来无关。这几条的音频走
 * `pinyinbare.a/o/e/i/u/ü` —— 录的正是**无调**的本音。
 */
...
export const COMPOUND_FINALS: readonly Syllable[] = [
  ...
  //    全部标 soundOnly：不进「听音选字」的选项。
  //
  //    ⭐ 音频已经全部是真人录音，所以这几条**不再有 TTS 读错的风险**；
  //    `char` 只剩下兜底文本与选题两个用途（见文件头）。
  //    拼音墙那边更彻底：`pinyinbare.ün` 录的是**无调**的韵母，连「韵」的四声都不沾了
  ...
  // ⭐ ong：汉语里它不能独立成音节，也没有对应的呼读字，所以始终没有干净的载体字。
  //    音频走 `pinyinbare.ong`（真人录的韵母本音），拼音墙**已经不借例词「松」**了。
  //    `char` 仍留空 → 不进题库：那面墙是「教」的地方，题库是「练」的地方
...
/**
 * 缺汉字载体的音节 —— 现在只剩 `ong` 一条。
 *
 * ⚠️ 这个名字来自 TTS 时代：那时「没有载体字」= 「音频必然念不准」。
 * 音频早已是真人录音（`pinyinbare.ong` 就在拼音墙上用着），
 * 所以**它现在表达的不再是「缺音频」**，而是**「不进题库」**——
 * `isUsable()` 仍然只看 `char`，判据没有跟着改。
 *
 * 想让 `ong` 进题库，得补一个读音恰好是 `ōng` 的常用单音字；
 * 汉语里没有这样的字（它不能独立成音节），所以它大概会一直留在这里。
 *
 * > 顺带一提：`npm run pinyin:record` 那个家长录音页已经名存实亡
 * > （它产出 webm，而播放器只认 mp3），不必再用了。
 */
