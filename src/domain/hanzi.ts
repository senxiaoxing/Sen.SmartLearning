 ...
 * 而不是把三个字连读成一个词组。
 *
 * ⚠️ **末尾**那个句号有个副作用：最后一个字落在句末，会被 TTS 的收尾降调压一下。
 * 一声（高平调）最吃亏——上机复听发现「黑」念成 hèi、「多」念成 duò。
 * 修正在 `scripts/generate-voices.mjs` 的 `TAIL_FIX_KEYS`（把末尾句号换成逗号），
 * **只对报回来的那几个字生效**。
 *
 * ⛔ 不要把修正搬到这个函数里：它一改，300 个字的文本全变，
 * 台账会判定「念的文本变了」而整批重生成——等于把已经念对的 298 个重新赌一次。
 * 这一层是「这个 TTS 引擎的怪癖」，属于生成期的知识，不属于 domain。
 *
 * @param card - 识字卡
 * @returns 朗读文本
 *
