...
describe('本音片段', () => {
  it('每个声母的本音都指向 bareKey 算出来的那一条', () => {
    for (const s of INITIALS) {
      const letter = initialOf(s.base)
      expect(pinyinCallName(letter), `声母 ${letter} 的本音对不上`).toBe(bareKey(letter))
    }
  })

  it('每个韵母的本音都指向 bareKey 算出来的那一条', () => {
    for (const s of [...SINGLE_FINALS, ...COMPOUND_FINALS]) {
      // ong 刻意不进表——见 pinyinCallName.ts 的说明
      if (s.char === undefined) continue
      expect(pinyinCallName(s.base), `韵母 ${s.base} 的本音对不上`).toBe(bareKey(s.base))
    }
  })
