...
export type MisconceptionTag =
  ...
  | 'column_misaligned'
  /**
   * **连续进位漏掉一次**：`68 + 75` 个位进 1 之后，十位相加又满十却没再进。
   *
   * 与 `no_carry` 是两种病：那个是压根不知道要进位，
   * 这个是会进位、只在**第二次**该进的时候忘了——多半是注意力而非理解问题。
   * 补救是让她在竖式上把每个进位的小 1 都写出来，别在心里记。
   */
  | 'carry_chain'
  /** 连续退位漏掉一次。与 `no_borrow` 的关系同 `carry_chain` 之于 `no_carry` */
  | 'borrow_chain'
