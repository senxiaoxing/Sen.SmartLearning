/**
 * @file 拼音录音页生成 —— ⛔ **已废弃，别再用它了**
 *
 * ## 为什么废弃（2026-09）
 *
 * 这个页面的全部理由是「TTS 念不准那几个没有载体字的音节，只能人录」。
 * 而拼音音频早在 design/11 里整体换成了**外部真人录音素材**
 * （`npm run pinyin:voice`，含 47 条声母韵母本音），那个前提已经不成立：
 * `eng` / `ong` / `ei` / `ün` 全都有干净的录音了。
 *
 * ## 就算要用，它也不工作
 *
 * 页面用 MediaRecorder 录出的是 **webm**，而播放器只认 `.mp3`
 * （`src/platform/speechClips.ts` 的 `loadClipMedia` 把 Blob 类型写死成
 * `audio/mpeg`、回退路径也写死 `.mp3`）。录完还得自己转码改名，
 * 而转完也没有任何地方会用到它——题库仍然只看 `char`。
 *
 * ## 留着它做什么
 *
 * 只当一份历史记录。真要让某个音节进题库，正确做法是给它补一个
 * 读音对得上的 `char`（见 `pinyinSyllables.ts` 的三条选字规矩），
 * 而不是录一条没有人会去接线的音频。
 *
 * 用法（不推荐）：npm run pinyin:record
 */
